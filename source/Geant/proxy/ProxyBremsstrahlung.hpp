...
template <>
struct Model_traits<ProxyBremsstrahlung> {
...
public:
  ...
  {
    return fDataManager->GetTable(ProxyPhysicsTableIndex::kLambda_eBrem_eminus)
        ->Value(index, energy);
  }
  ...
  {
    return fDataManager->GetTable(ProxyPhysicsTableIndex::kDEDX_eBrem_eminus)
        ->Value(index, energy);
  }
};
