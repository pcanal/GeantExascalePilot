...
void ProxyPhysicsVector::Relocate(void *devPtr)
{
  ...
  cudaMemcpy(d_binVector, fBinVector, sizeof(double) * fNumberOfNodes,
             cudaMemcpyHostToDevice);
  cudaMemcpy(d_dataVector, fDataVector, sizeof(double) * fNumberOfNodes,
             cudaMemcpyHostToDevice);
  cudaMemcpy(d_secDerivative, fSecDerivative, sizeof(int) * fNumberOfNodes,
             cudaMemcpyHostToDevice);
...
void ProxyPhysicsVector::ComputeSecDerivatives()
{
  ...
  for (int i = 1; i < n; ++i) {
    fSecDerivative[i] =
        3.0 *
        ((fDataVector[i + 1] - fDataVector[i]) / (fBinVector[i + 1] - fBinVector[i]) -
         (fDataVector[i] - fDataVector[i - 1]) / (fBinVector[i] - fBinVector[i - 1])) /
                                                  ...
                                                  double endPointDerivative)
{
  ...
  for (int i = 1; i < n; ++i) {
    sig = (fBinVector[i] - fBinVector[i - 1]) / (fBinVector[i + 1] - fBinVector[i - 1]);
    p   = sig * (fSecDerivative[i - 1]) + 2.0;
    fSecDerivative[i] = (sig - 1.0) / p;
    u[i] = (fDataVector[i + 1] - fDataVector[i]) / (fBinVector[i + 1] - fBinVector[i]) -
           (fDataVector[i] - fDataVector[i - 1]) / (fBinVector[i] - fBinVector[i - 1]);
    u[i] = 6.0 * u[i] / (fBinVector[i + 1] - fBinVector[i - 1]) - sig * u[i - 1] / p;
  }

  sig = (fBinVector[n - 1] - fBinVector[n - 2]) / (fBinVector[n] - fBinVector[n - 2]);
  p   = sig * fSecDerivative[n - 2] + 2.0;
  un  = (6.0 / (fBinVector[n] - fBinVector[n - 1])) *
           (endPointDerivative -
            (fDataVector[n] - fDataVector[n - 1]) / (fBinVector[n] - fBinVector[n - 1])) -
       u[n - 1] / p;
  ...
  for (int k = n - 1; k > 0; --k) {
    fSecDerivative[k] *=
        (fSecDerivative[k + 1] - u[k] * (fBinVector[k + 1] - fBinVector[k - 1]) /
...
void ProxyPhysicsVector::FillSecondDerivatives()
{
  ...
  u[n - 1] =
      (fDataVector[n] - fDataVector[n - 1]) / (fBinVector[n] - fBinVector[n - 1]) -
      (fDataVector[n - 1] - fDataVector[n - 2]) / (fBinVector[n - 1] - fBinVector[n - 2]);
  ...
  fSecDerivative[n] = (fSecDerivative[n - 1] - (1.0 - sig) * fSecDerivative[n - 2]) / sig;
  sig = 1.0 - ((fBinVector[2] - fBinVector[1]) / (fBinVector[2] - fBinVector[0]));
  fSecDerivative[1] *= (fSecDerivative[2] - u[1] / (1.0 - sig));
  fSecDerivative[0] = (fSecDerivative[1] - sig * fSecDerivative[2]) / (1.0 - sig);
...
int ProxyPhysicsVector::FindBinLocation(double energy)
{
  ...
  if (bin > 0 && energy < fBinVector[bin]) {
    --bin;
  }
  return bin;
}
...
double ProxyPhysicsVector::SplineInterpolation(double energy, int lastBin)
{
  ...
  double res = a * y1 + b * y2 +
               ((a * a * a - a) * fSecDerivative[lastBin] +
                (b * b * b - b) * fSecDerivative[lastBin + 1]) *
                   delta * delta / 6.0;
