...
ProxySeltzerBerger::ProxySeltzerBerger()
{
  ...
  lpmEnergy = xiLPM = phiLPM = gLPM = z13 = z23 = lnZ = Fel = Finel = fCoulomb = fMax = 0;
...
int ProxySeltzerBerger::SampleSecondaries(TrackState *track)
{
  ...
  if (isElectron && x0 < 0.97 &&
    ...
    double ylim = Math::Min(fDataSB[Z].Value(0.97, 4 * Math::Log(10.)),
...
ThreeVector ProxySeltzerBerger::SamplePhotonDirection(double energyIn)
{
  ...
  ThreeVector gammaDirection(sinTheta * Math::Cos(phi), sinTheta * Math::Sin(phi),
                             cosTheta);
...
void ProxySeltzerBerger::CalcLPMFunctions(double k, double totalEnergy)
{
  ...
  if (s0 < 0.1) {
    ...
    phiLPM = 1. - Math::Exp(-6. * s0 * (1. + (3. - Math::Pi()) * s0) +
                            s3 / (0.623 + 0.795 * s0 + 0.658 * s2));
    if (s0 < 0.415827397755) {
      // using eq.77 approxim. valid 0.07<s<2
      double psiLPM =
          1 - Math::Exp(-4 * s0 -
                        8 * s2 / (1 + 3.936 * s0 + 4.97 * s2 - 0.05 * s3 + 7.50 * s4));
      gLPM = 3 * psiLPM - 2 * phiLPM;
    } else {
      // using alternative parametrisiation
      double pre = -0.16072300849123999 + s0 * 3.7550300067531581 +
                   s2 * -1.7981383069010097 + s3 * 0.67282686077812381 +
                   s4 * -0.1207722909879257;
      gLPM = Math::Tanh(pre);
    }
  } else {
...
GEANT_HOST
bool ProxySeltzerBerger::RetrieveSeltzerBergerData(std::ifstream &in,
                                                   Proxy2DVector *vec2D)
{
  // binning
  int k;
