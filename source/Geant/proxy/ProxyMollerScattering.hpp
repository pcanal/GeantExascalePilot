...
public:
  ...
  ProxyMollerScattering(const ProxyMollerScattering &model)
      : ProxyEmModel<ProxyMollerScattering>()
  {
...
double ProxyMollerScattering::CrossSectionPerAtom(double Z, double kineticEnergy)
{
  ...
  xsec      = ((xmax - xmin) *
              (1.0 - gg + 1.0 / (xmin * xmax) + 1.0 / ((1.0 - xmin) * (1.0 - xmax))) -
...
int ProxyMollerScattering::SampleSecondaries(TrackState *track)
{
  ...
  double totalMomentum =
      Math::Sqrt(kineticEnergy * (kineticEnergy + 2.0 * clhep::electron_mass_c2));
  double deltaMomentum =
      Math::Sqrt(deltaKinEnergy * (deltaKinEnergy + 2.0 * clhep::electron_mass_c2));
  double cost = deltaKinEnergy * (energy + clhep::electron_mass_c2) /
