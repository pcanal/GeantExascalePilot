...
template <>
struct Model_traits<ProxyIonization> {
...
public:
  ...
  {
    return fDataManager->GetTable(ProxyPhysicsTableIndex::kLambda_eIoni_eminus)
        ->Value(index, energy);
  }
  ...
  {
    return fDataManager->GetTable(ProxyPhysicsTableIndex::kDEDX_eIoni_eminus)
        ->Value(index, energy);
  }

  double GetRange(int index, double energy)
  {
    return fDataManager->GetTable(ProxyPhysicsTableIndex::kRange_eIoni_eminus)
        ->Value(index, energy);
  }

  double GetInverseRange(int index, double energy)
  {
    return fDataManager->GetTable(ProxyPhysicsTableIndex::kInverseRange_eIoni_eminus)
        ->Value(index, energy);
  }
};
...
double ProxyIonization::AlongStepGPIL(TrackState *track)
{
  ...
  stepLimit = (range > finR) ? range * data::dRoverRange +
                                   finR * (1.0 - data::dRoverRange) * (2.0 * finR / range)
                             : range;
